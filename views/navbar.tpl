	...
	<div class="container-fluid">
		...
		<div class="collapse navbar-collapse" id="navbar">
			<table class="table table-condensed">
				...
				<tr>
					{{ range $k,$v := .spacedisk}} {{ if compare_not $k "0"}}
			...
			<ul class="nav navbar-nav">
				...
				<li>
					...
					<a data-toggle="modal" data-target="#nonlues" href="#">
						Non lues <span class="badge">{{ .numNonLues }}</span>
					</a>
				</li>
			</ul>
...
<div class="modal fade" id="nonlues" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<form method="POST" class="form-horizontal" action="/chown">
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
					<h4 class="modal-title" id="myModalLabel">Fichier non vues</h4>
				</div>
				<div class="modal-body">
					...
					<fieldset>
						<div class="list-group">
							{{ range $k,$v := .nonLues }}
							<a href="/list/{{ $v.Url }}" class="list-group-item navbar-one-movie">
							{{ $v.Name }}
								{{ if eq $v.Url "."}}
								<button type="button" class="btn btn-link navbar-del badge" href="/delete/{{ $v.Name }}">
								{{ else }}
								<button type="button" class="btn btn-link navbar-del badge" href="/delete/{{ $v.Url }}/{{ $v.Name }}">
								{{ end }}
									<span class="glyphicon glyphicon-trash"></span>
								</button>
								{{ if eq $v.Url "."}}
								<button type="button" class="btn btn-link navbar-vu badge" href="/vues/{{ $v.Name }}">
								{{ else }}
								<button type="button" class="btn btn-link navbar-vu badge" href="/vues/{{ $v.Url }}/{{ $v.Name }}">
								{{ end }}
				...
				<div class="modal-footer">
					...
					<button type="button" class="btn btn-default close-modal" data-dismiss="modal">Close and Refresh</button>
